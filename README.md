...
- **`in <slot>`** — where the stray pointer was found: `pathlist[1]`, `partial_pathlist[0]`, `cheapest_startup_path`, `cheapest_total_path`, `cheapest_parameterized_paths[2]`, or a compound-path field like `SortPath.subpath`. List slots carry their index; scalar slots do not.
